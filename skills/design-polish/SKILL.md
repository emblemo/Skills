---
name: design-polish
description: "Automates a stylistic 'design pass' after the first iteration of a UI, polishing an existing product to match the consistent standard described in DESIGN.MD, without touching the layout. Use this skill when the user has finished a first iteration or prototype of a UI and says things like 'polish the UI', 'do a design pass', 'make the styling consistent', 'apply the design system'; when the user mentions wrapping up or finishing an iteration of a project in a frontend or UI context, even without explicitly asking for styling, if the project has a freshly built interface; or when DESIGN.MD already exists in the project or should be created as the source of truth for its look. The skill checks whether DESIGN.MD already exists, audits the UI against it as covered, missing, or conflicting, always asks the user before introducing anything ambiguous, applies styling without changing structure or layout, and finally updates DESIGN.MD so it stays a complete and current standard."
---
